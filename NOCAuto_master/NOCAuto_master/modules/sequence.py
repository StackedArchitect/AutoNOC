...
class NOCSequence(uvm_sequence):
    ...
    def generate_source_destination(self):
        ...
        config=self.helper.algorithmic_parameters
        print(f"self.helper.huristic --------------------------->{self.helper.huristic }")
        if self.helper.huristic =="DE":
            traffic= generate_source_destination_DE(row,column,int(config["gen"]),int(config["popsize"]),float(config["crossover_rate"]),float(config["mutation_rate"]))
        elif self.helper.huristic=="PSO":
            traffic=source_destination_generator_pos(row,column,int(config["iter_size"]),int(config["pop_size"]))
        elif self.helper.huristic=="GA":
            traffic=source_destination_generator_GA(row,column,int(config["n_iter"]),int(config["n_pop"]),float(config["r_cross"]),float(config["r_mut"]))
        else:
